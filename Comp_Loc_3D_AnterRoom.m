...
% -----------------------------------------------------------
%   Experiment Setup in anter room
% -----------------------------------------------------------
...
%  - 17 example test points in anterroom scenario
%  - Requires JSON data loading support

clc; 
clear;
addpath('./data/AnterRoom/');
addpath('./settings');
addpath('./functions');
...
Ele = 5:2:80;             % Elevation search grid (degrees)
...
oversamplingFactor = 5;       % Spectrum oversampling ratio
bandwidth = 1e6;              % Signal bandwidth [Hz]
mapWidth = [5.80, 3.50];      % Spatial search boundaries (m)
x_bound_DPD = 12.10;          % X-axis search limit (m)
y_bound_DPD = 9.50;           % Y-axis search threshold (m)
...
SelectedPots = 40:56;                   % Selected test points
...
for pot_n = 1:numPots
    ...
    Pot = Points_Anterroom(potn,:);       % Ground truth coordinates
    % fprintf(['\n ', potstr,', oF = ', num2str(oversamplingFactor),'，  ', num2str(Pot(1:2)), '\n']);

    % File pattern matching for current test point
    if potn<49
        potstr_anterroom = ['P', num2str(potn)];
    elseif potn<52  % potn = 49,50,51
        potstr_anterroom = ['P', num2str(potn+2)];
    else
        potstr_anterroom = ['P', num2str(potn+4)];
    end
    [P_AllFiles, P_stationIndices] = findfiles('./data/AnterRoom', ID_Stations, potstr_anterroom);

    %% ------------------------------------------------------
    %   Station Configuration
    % -------------------------------------------------------
    P_stationIndices = [3,4,5,6];  % Selected base stations
    ...
    for n = 1:len_Stations
        ...
        IQ_list_cell{stan_temp} = loadjson(P_AllFiles{P_stationIndices(n)-2});
